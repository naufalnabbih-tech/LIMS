...
class ChemicalReference extends Component
{
    use WithPagination;
    ...
    {
        $this->chemicals = Material::select('id', 'name')
    ...
    public $name = '';
    public $material_id = '';
    public $chemicalSearch = '';
    public $showChemicalDropdown = false;
    ...
    protected function rules()
    {
        $rules = [
            ...
            'selectedSpecifications' => 'required|array|exists:specifications,id',
    ...
    public function render()
    {
        ...
        foreach ($references->items() as $reference) {
            $chemicalName = $reference->material->name;
            if (!$groupedReferences->has($chemicalName)) {
                $groupedReferences->put($chemicalName, collect());
            }
    ...
    {
        if ($this->chemicals->isEmpty()) {
            session()->flash('error', 'Tidak dapat menambah reference. Silakan tambahkan chemical terlebih dahulu.');
            return;
        }
    ...
    public function openEditModal($id)
    {
        ...
        $this->chemicalSearch = $reference->material->name;
    ...
    public function store()
    {
        ...
        try {
            ...
            if ($existingReference) {
                $this->addError('name', 'Reference dengan nama ini untuk chemical yang sama sudah ada.');
                return;
            }
    ...
    public function getFilteredChemicalsProperty()
    {
        if (empty($this->chemicalSearch)) {
            return $this->chemicals;
        }

        return $this->chemicals->filter(function ($chemical) {
            return stripos($chemical->name, $this->chemicalSearch) !== false;
        });
    }

    public function selectChemical($id, $name)
    {
        $this->material_id = $id;
        $this->chemicalSearch = $name;
        $this->showChemicalDropdown = false;
    ...
    public function updatedChemicalSearch()
    {
        $this->showChemicalDropdown = !empty($this->chemicalSearch);
        if (empty($this->chemicalSearch)) {
            $this->material_id = '';
        }
    }

    public function openChemicalDropdown()
    {
        $this->showChemicalDropdown = true;
    }

    public function closeChemicalDropdown()
    {
        $this->showChemicalDropdown = false;
    }
