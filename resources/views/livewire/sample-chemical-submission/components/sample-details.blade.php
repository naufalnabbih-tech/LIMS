<div>
    @if ($show && $sample)
        <div class="fixed inset-0 bg-gray-900/75 overflow-y-auto h-full w-full z-50" x-data="{ show: true }" x-show="show"
            ...
            <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-2/3 xl:w-1/2 shadow-lg rounded-md bg-white"
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-900">{{ $sample->batch_lot ?? 'N/A' }}</p>
                            </div>
                        </div>
                    ...
                    @if ($sample->notes)
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Sample Notes</label>
                    ...
                    @if ($sample->review_notes)
                        <div class="mt-4">
                            <label
                                class="block text-sm font-medium mb-1 @if ($sample->status_label == 'Rejected') text-red-700 @else text-green-700 @endif">
                                {{ $sample->status_label }} Notes
                            </label>

                            @switch($sample->status_label)
                                @case('Rejected')
                                    <div class="pb-2 bg-red-50 rounded-md border border-red-200">
                                        <div class="flex">
                                            <div class="ml-3">
                                                <div class="mt-2 text-sm text-red-700">
                                                    <p>{{ $sample->review_notes }}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                @case('Approved')
                                    <div class="pb-2 bg-green-50 rounded-md border border-green-200">
                                        <div class="flex">
                                            <div class="ml-3">
                    ...
                    @if ($currentStatusName !== 'pending' && $sample->analysis_method)
                        ...
                        @if ($acceptedHandover)
                            ...
                            <div class="mt-6">
                                <label class="block text-sm font-medium text-gray-700 mb-3">
                                    ...
                                    <span class="ml-2 px-2 py-1 text-xs bg-gray-100 text-gray-600 rounded">Before
                                        Handover</span>
                                </label>
                                <div
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <!-- Analysis Method -->
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700">Analysis
                                                Method</label>
                                        ...
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700">Primary
                                                Analyst</label>
                                            <p class="mt-1 text-sm font-semibold text-gray-900">
                                        ...
                                        @if ($sample->analysis_method === 'joint' && $sample->secondary_analyst_id)
                                            <div>
                                                <label class="block text-sm font-medium text-gray-700">Secondary
                                                    Analyst</label>
                                                <p class="mt-1 text-sm font-semibold text-gray-900">
                                        ...
                                        @if ($acceptedHandover->submitted_at)
                                            <div>
                                                <label class="block text-sm font-medium text-gray-700">Handed Over
                                                    At</label>
                                        ...
                                        <!-- Handover Reason -->
                                        @if ($acceptedHandover->reason)
                                            <div class="md:col-span-2">
                                                <label class="block text-sm font-medium text-gray-700">Handover
                                                    Reason</label>
                                                <div class="mt-1 p-3 bg-white rounded-md border border-orange-200">
                                                    <p class="text-sm text-gray-900">{{ $acceptedHandover->reason }}
                                                    </p>
                                                </div>
                                            </div>
                                        @endif
                                    ...
                                    @if ($sample->analysis_started_at && $acceptedHandover->submitted_at)
                                        <div class="mt-4 pt-4 border-t border-blue-200">
                                            <label class="block text-sm font-medium text-gray-700">Duration (Until
                                                Handover)</label>
                                            @php
                                                $duration = $sample->analysis_started_at->diff(
                                                    $acceptedHandover->submitted_at,
                                                );
                                                $durationText = '';
                                                if ($duration->days > 0) {
                                                    $durationText .=
                            ...
                            <div class="mt-6">
                                <label class="block text-sm font-medium text-gray-700 mb-3">
                                    ...
                                    <span class="ml-2 px-2 py-1 text-xs bg-green-100 text-green-700 rounded">After
                                        Takeover</span>
                                </label>
                                <div
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        @if ($acceptedHandover->taken_at)
                                            <div>
                                                <label class="block text-sm font-medium text-gray-700">Taken Over
                                                    At</label>
                                    ...
                                    @if ($acceptedHandover->taken_at && $sample->analysis_completed_at)
                                        <div class="mt-4 pt-4 border-t border-green-200">
                                            <label class="block text-sm font-medium text-gray-700">Duration (After
                                                Takeover)</label>
                                            @php
                                                $duration = $acceptedHandover->taken_at->diff(
                                                ...
                                                $durationText = trim($durationText) ?: 'Less than a minute';
                                            @endphp
                                            <p class="mt-1 text-sm font-semibold text-green-600">{{ $durationText }}
                                            </p>
                                        </div>
                                    @elseif ($acceptedHandover->taken_at && !$sample->analysis_completed_at)
                                        <div class="mt-4 pt-4 border-t border-green-200">
                                            <label class="block text-sm font-medium text-gray-700">Analysis In
                                                Progress</label>
                                            @php
                                                ...
                                                if ($elapsed->h > 0) {
                                                    $elapsedText .=
                                                        $elapsed->h . ' hour' . ($elapsed->h > 1 ? 's' : '') . ' ';
                                                }
                                                if ($elapsed->i > 0) {
                                                    $elapsedText .=
                                                        $elapsed->i . ' minute' . ($elapsed->i > 1 ? 's' : '');
                                                }
                                                $elapsedText = trim($elapsedText) ?: 'Just started';
                                            @endphp
                                            <p class="mt-1 text-sm font-semibold text-orange-600">Running for
                                                {{ $elapsedText }}</p>
                                        </div>
                                    @endif
                                </div>
                        ...
                        @else
                            ...
                            <div class="mt-6">
                                <label class="block text-sm font-medium text-gray-700 mb-3">Analysis
                                    Information</label>
                                <div
                                    class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200 p-4">
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    @if (in_array($currentStatusName, ['analysis_completed', 'reviewed', 'approved', 'rejected']) &&
                        ...
                        <div class="mt-6">
                            ...
                            <div
                                    ...
                                    @foreach ($sample->testResults->groupBy('parameter_name') as $parameterName => $results)
                                        <div class="bg-white rounded-lg p-4 border border-green-200">
                                            ...
                                            @php
                                                ...
                                                $operator = $firstResult->spec_operator ?? null;

                                                // Check if this is a should_be operator (text value)
                                                if ($operator === 'should_be') {
                                                    // Use text value for should_be operator
                                                    $displayValue = $firstResult->test_value_text ?? 'N/A';
                                                    $isNumeric = false;
                                                } else {
                                                    // Calculate average for numeric values
                                                    $displayValue = $results->avg('test_value');
                                                    $isNumeric = true;
                                                }
                                            ...
                                            <div class="flex items-center justify-between mb-2">
                                                <span class="text-lg font-bold text-gray-900">
                                                    @if ($isNumeric)
                                                        {{ rtrim(rtrim(number_format($displayValue, 4, '.', ''), '0'), '.') }}
                                                    @else
                                                        {{ $displayValue }}
                                                    @endif
                                                </span>
                                ...
                                @php
                                    $firstTestResult = $sample->testResults->first();
                                    $analysisNotes = $firstTestResult ? $firstTestResult->notes : null;
                                @endphp
                                @if ($analysisNotes)
                                    <div class="mt-4 pt-4 border-t border-green-200">
                                        <label class="block text-sm font-medium text-gray-700">Analysis Notes</label>
                                        <p class="mt-1 text-sm text-gray-600">{{ $analysisNotes }}</p>
                                    </div>
                                @endif
                            </div>
                        </div>
                                        ...
                                        @foreach ($statusHistory as $index => $history)
                                            <tr class="{{ $index % 2 == 0 ? 'bg-white' : 'bg-gray-50' }}">
                                                ...
                                                <td class="px-4 py-3 whitespace-nowrap">
                                                    <span
                                                        class="inline-flex px-2 py-1 text-xs font-semibold rounded-full {{ $history['status_color'] ?? 'bg-gray-100 text-gray-800' }}">
                                                        {{ $history['status'] }}
                                                    </span>
                                                </td>
                                                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-gray-200">
                        <button wire:click="close"
                            class="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-800 text-sm font-medium rounded-lg transition-colors duration-200 cursor-pointer">
                            Close
                        </button>
                        <button wire:click="printSampleLabel"
                            class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg transition-colors duration-200 flex items-center space-x-2 cursor-pointer">
