        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                ...
                <div class="flex space-x-3">
                    ...
                    @endif
                    @if ($canApprove)
                        <button wire:click="approveSample"
                            wire:confirm="Are you sure you want to approve this sample?"
                            class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg transition-colors duration-200">
                            Approve Sample
                        </button>
                        <button wire:click="rejectSample"
                            wire:confirm="Are you sure you want to reject this sample?"
                ...
                <div class="bg-white shadow-sm rounded-lg border border-gray-200">
                    ...
                    <div class="px-6 py-4 space-y-4">
                        ...
                        <div>
                            ...
                            <span class="inline-flex px-3 py-1 text-sm font-semibold rounded-full {{ $sample->status_color }}">
                                {{ $sample->status_label }}
                            </span>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-900">{{ $sample->rawMaterial->name ?? 'N/A' }}</p>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-900">{{ $sample->submission_time->format('M d, Y \a\t H:i') }}</p>
                        </div>
                        ...
                        <div>
                            ...
                            @if ($sample->has_coa && $sample->coa_file_path)
                                <div class="flex items-center gap-2">
                                    ...
                                    <a href="{{ asset('storage/' . $sample->coa_file_path) }}" target="_blank" class="text-sm text-blue-600 hover:text-blue-800 underline">View CoA File</a>
                                </div>
                            @else
                        ...
                        <div class="bg-white shadow-sm rounded-lg border border-gray-200">
                            ...
                            <div class="px-6 py-4 space-y-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm font-semibold text-gray-900">{{ ucfirst($sample->analysis_method) }}</p>
                                </div>
                                ...
                                @if ($sample->primary_analyst_id)
                                    <div>
                                        ...
                                        <p class="text-sm font-semibold text-gray-900">{{ $sample->primaryAnalyst->name ?? 'N/A' }}</p>
                                    </div>
                                @endif
                                ...
                                @if ($sample->analysis_method === 'joint' && $sample->secondary_analyst_id)
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Secondary Analyst</label>
                                        <p class="text-sm font-semibold text-gray-900">{{ $sample->secondaryAnalyst->name ?? 'N/A' }}</p>
                                    </div>
                                @endif
                                ...
                                @if ($sample->analysis_started_at)
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-900">{{ $sample->analysis_started_at->format('M d, Y \a\t H:i') }}</p>
                                    </div>
                                @endif
                                ...
                                @if ($sample->analysis_completed_at)
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Analysis Completed</label>
                                        <p class="text-sm text-gray-900">{{ $sample->analysis_completed_at->format('M d, Y \a\t H:i') }}</p>
                                    </div>

                                    <!-- Duration -->
                                    @if ($sample->analysis_started_at && $sample->analysis_completed_at)
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 mb-1">Analysis Duration</label>
                                            @php
                                                $duration = $sample->analysis_started_at->diff($sample->analysis_completed_at);
                                                $durationText = '';
                                                if ($duration->days > 0) $durationText .= $duration->days . ' day' . ($duration->days > 1 ? 's' : '') . ' ';
                                                if ($duration->h > 0) $durationText .= $duration->h . ' hour' . ($duration->h > 1 ? 's' : '') . ' ';
                                                if ($duration->i > 0) $durationText .= $duration->i . ' minute' . ($duration->i > 1 ? 's' : '');
                    ...
                    <div class="px-6 py-6">
                        @if (!empty($analysisResults))
                            ...
                            <div class="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                                @php
                                    $totalTests = collect($analysisResults)->sum(fn($r) => count($r['test_results'] ?? []));
                                    $passedTests = count(array_filter($analysisResults, fn($r) => $r['status'] === 'pass'));
                                    $failedTests = count(array_filter($analysisResults, fn($r) => $r['status'] === 'fail'));
                                @endphp
                            ...
                            <div class="space-y-6">
                                @foreach ($analysisResults as $parameter => $data)
                                    <div class="border border-gray-200 rounded-lg overflow-hidden
                                        {{ $data['status'] === 'pass' ? 'border-green-200' :
                                           ($data['status'] === 'fail' ? 'border-red-200' : 'border-gray-200') }}">

                                        <!-- Parameter Header -->
                                        <div class="px-4 py-3 bg-gray-50 border-b border-gray-200">
                                            <div class="flex items-center justify-between">
                                                <div>
                                                    <h4 class="text-lg font-semibold text-gray-900">{{ $data['spec_name'] }}</h4>
                                                    <p class="text-sm text-gray-600">Specification: {{ $data['spec'] }}</p>
                                                </div>
                                                <div class="flex items-center space-x-2">
                                                    @if ($data['status'] === 'pass')
                                            ...
                                            @if (!empty($data['test_results']))
                                                    ...
                                                    @foreach ($data['test_results'] as $result)
                                                        <div class="p-4 border rounded-lg
                                                            ...
                                                            <div class="flex items-center justify-between mb-2">
                                                                <span class="text-xs font-medium text-gray-500">Reading #{{ $result['reading_number'] }}</span>
                                                            ...
                                                            <div class="text-xs text-gray-500 space-y-1">
                                                                ...
                                                                @if ($result['notes'])
                                                                    <div class="mt-2 p-2 bg-white rounded border">
                                                                        <span class="font-medium">Notes:</span> {{ $result['notes'] }}
                                                                    </div>
                                                                @endif
                                                            </div>
                                                        </div>
                                                ...
                                                <div class="mt-4 pt-3 border-t border-gray-200">
                                                    <div class="text-sm text-gray-600">
                                                        <strong>{{ count($data['test_results']) }}</strong> readings completed
                                                        @php
                                                            $passedReadings = collect($data['test_results'])->where('passes', true)->count();
                                                        @endphp
                                                        • <span class="text-green-600">{{ $passedReadings }} passed</span>
                                                        • <span class="text-red-600">{{ count($data['test_results']) - $passedReadings }} failed</span>
                                                    </div>
                                                </div>
                                            @else
                                                <div class="text-center py-8">
                                                    ...
                                                    <p class="mt-2 text-sm text-gray-500">No test results available for this parameter</p>
                                                </div>
                                            @endif
                                        </div>
                                    </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-100">
                                    <tr>
                                        ...
                                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Interval</th>
                                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Analyst</th>
                                    </tr>
                                </thead>
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach ($statusHistory as $index => $history)
                                        <tr class="{{ $index % 2 == 0 ? 'bg-white' : 'bg-gray-50' }}">
                                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">{{ $history['id'] }}</td>
                                            ...
                                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                                                @if ($history['previous_time'])
                                                    @php
                                                        ...
                                                        $intervalText = '';
                                                        if ($interval->d > 0) $intervalText .= $interval->d . 'd ';
                                                        if ($interval->h > 0) $intervalText .= $interval->h . 'h ';
                                                        if ($interval->i > 0) $intervalText .= $interval->i . 'm';
                                                        if (empty($intervalText)) $intervalText = '< 1m';
                                                    @endphp
                                                    {{ trim($intervalText) }}
                                                @else
                                            ...
                                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">{{ $history['analyst'] }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
    ...
    @if ($showApprovalForm)
        <div class="fixed inset-0 bg-black/50 backdrop-blur-sm overflow-y-auto h-full w-full z-50"
            ...
            <div class="relative top-4 mx-auto my-4 w-11/12 md:w-2/3 lg:w-1/2 max-w-2xl shadow-2xl rounded-2xl bg-white overflow-hidden"
                ...
                <!-- Modal Header -->
                <div class="bg-gradient-to-r {{ $approvalAction === 'approve' ? 'from-green-500 to-green-600' : 'from-red-500 to-red-600' }} px-6 py-4 flex items-center justify-between">
                ...
                <div class="bg-gray-50 px-6 py-4 border-t border-gray-200 flex justify-end space-x-3">
                    ...
                    <button type="button" wire:click="submitReview" wire:loading.attr="disabled"
                        ...
                        <span wire:loading.remove>{{ $approvalAction === 'approve' ? 'Approve Sample' : 'Reject Sample' }}</span>
