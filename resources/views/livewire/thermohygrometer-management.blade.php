    ...
    <div class="flex flex-col">
        ...
        <div class="bg-white shadow-sm rounded-lg border border-gray-200 mb-4">
            <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                <div>
                    <h2 class="text-xl font-semibold text-gray-900">Thermohygrometer Management</h2>
                    <p class="text-sm text-gray-600 mt-1">Manage and organize laboratory thermohygrometers</p>
                </div>
                <div>
        ...
        <div class="bg-white shadow-sm rounded-lg border border-gray-200">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($thermohygrometers as $index => $thermohygrometer)
                            <tr wire:key="thermohygrometer-{{ $thermohygrometer->id }}"
                                class="hover:bg-gray-50 transition-colors duration-150">
                                <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">
                                    {{ $thermohygrometers->firstItem() + $index }}
                                </td>
                                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                                    <div>
                                        <div class="font-medium">{{ $thermohygrometer->name }}</div>
                                        @if($thermohygrometer->description)
                                            <div class="text-gray-500 text-xs">{{ Str::limit($thermohygrometer->description, 50) }}</div>
                                        @endif
                                    </div>
                                </td>
                                <td class="px-4 py-3 whitespace-nowrap text-right text-sm">
                                    <div class="flex justify-end space-x-2">
                                        <button wire:click="openEditModal({{ $thermohygrometer->id }}, '{{ addslashes($thermohygrometer->name) }}', '{{ addslashes($thermohygrometer->description) }}')"
                                        ...
                                        <button wire:click="delete({{ $thermohygrometer->id }})"
                                            wire:confirm="Are you sure you want to delete this thermohygrometer?"
                            ...
                            <tr>
                                <td colspan="3" class="px-4 py-12 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <h3 class="text-lg font-medium text-gray-900 mb-2">No thermohygrometers found</h3>
                                        <p class="text-sm text-gray-500 mb-4">Get started by adding your first laboratory thermohygrometer</p>
                                        <button wire:click="openAddModal()"
            ...
            <!-- Pagination Footer -->
            @if ($thermohygrometers->hasPages())
                <div class="bg-white px-4 py-3 border-t border-gray-200 rounded-b-xl">
                    <div class="flex items-center justify-between">
                        <!-- Desktop Results Info -->
                        <div class="hidden sm:block">
                            <p class="text-sm text-gray-700">
                                Showing <span class="font-medium">{{ $thermohygrometers->firstItem() ?? 0 }}</span>-<span
                                    class="font-medium">{{ $thermohygrometers->lastItem() ?? 0 }}</span> of <span
                                    class="font-medium">{{ $thermohygrometers->total() }}</span> thermohygrometers
                            </p>
                        </div>
                        ...
                        <div class="sm:hidden">
                            <p class="text-sm text-gray-700">
                                Page {{ $thermohygrometers->currentPage() }} of {{ $thermohygrometers->lastPage() }}
                            </p>
                        </div>
                        ...
                        <div class="flex items-center space-x-2">
                            {{-- Previous Page Link --}}
                            @if ($thermohygrometers->onFirstPage())
                                <span
                            ...
                            <div class="hidden sm:flex items-center space-x-1">
                                @php
                                    $currentPage = $thermohygrometers->currentPage();
                                    $lastPage = $thermohygrometers->lastPage();
                            ...
                            {{-- Next Page Link --}}
                            @if ($thermohygrometers->hasMorePages())
            ...
            <div class="fixed inset-0 bg-gray-900/75 p-4 flex items-center justify-center z-50">
                <div class="relative w-full max-w-md bg-white rounded-2xl shadow-2xl p-8">
                    <div class="flex justify-between items-center pb-4 border-b">
                        <h3 class="text-2xl font-bold">Add New Thermohygrometer</h3>
                    ...
                    <div class="mt-6">
                        ...
                        <form wire:submit="store">
                            <div class="mb-5">
                                <label for="add-name" class="block text-sm font-bold mb-2">Thermohygrometer Name</label>
                            ...
                            <div class="mb-5">
                                <label for="add-description" class="block text-sm font-bold mb-2">Description (Optional)</label>
                                <textarea id="add-description" wire:model="description"
                                    class="shadow-sm border @error('description') border-red-500 @else border-gray-300 @enderror rounded-lg w-full py-3 px-4 h-24 resize-none"
                                    placeholder="Enter thermohygrometer description..."></textarea>
                                @error('description')
                                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            <div class="flex justify-end pt-5 border-t mt-6">
                                ...
                                <button type="submit" wire:loading.attr="disabled" wire:target="store"
                                    ...
                                    <span wire:loading.remove wire:target="store">Save Thermohygrometer</span>
        ...
        @if ($isEditModalOpen)
            <div class="fixed inset-0 bg-gray-900/75 p-4 flex items-center justify-center z-50"
                ...
                <div class="relative w-full max-w-md bg-white rounded-2xl shadow-2xl p-8">
                    <div class="flex justify-between items-center pb-4 border-b">
                        <h3 class="text-2xl font-bold">Edit Thermohygrometer</h3>
                    ...
                    <div class="mt-6">
                        ...
                        <form wire:submit="update">
                            <div class="mb-5">
                                <label for="edit-name" class="block text-sm font-bold mb-2">Thermohygrometer Name</label>
                            ...
                            <div class="mb-5">
                                <label for="edit-description" class="block text-sm font-bold mb-2">Description (Optional)</label>
                            ...
                            <div class="flex justify-end pt-5 border-t mt-6">
                                ...
                                <button type="submit" wire:loading.attr="disabled" wire:target="update"
                                    class="px-6 py-2 rounded-lg text-white bg-blue-500 hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer">
                                    <span wire:loading.remove wire:target="update">Update Thermohygrometer</span>
