    ...
    <div class="flex flex-col">
        ...
        <div class="bg-white shadow-sm rounded-lg border border-gray-200 mb-4">
            <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                <div>
                    <h2 class="text-xl font-semibold text-gray-900">Instrument Management</h2>
                    <p class="text-sm text-gray-600 mt-1">Manage and organize laboratory instruments</p>
                </div>
                <div>
                        ...
                        @forelse ($instruments as $index => $instrument)
                            <tr wire:key="instrument-{{ $instrument->id }}"
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">
                                    {{ $instruments->firstItem() + $index }}
                                </td>
                                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                                    <div>
                                        <div class="font-medium">{{ $instrument->name }}</div>
                                        @if($instrument->description)
                                            <div class="text-gray-500 text-xs">{{ Str::limit($instrument->description, 50) }}</div>
                                        @endif
                                    </div>
                                </td>
                                <td class="px-4 py-3 whitespace-nowrap text-right text-sm">
                                    <div class="flex justify-end space-x-2">
                                        <button wire:click="openEditModal({{ $instrument->id }}, '{{ addslashes($instrument->name) }}', '{{ addslashes($instrument->description) }}')"
                                        ...
                                        <button wire:click="delete({{ $instrument->id }})"
                                            wire:confirm="Are you sure you want to delete this instrument?"
                            ...
                            <tr>
                                <td colspan="3" class="px-4 py-12 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <h3 class="text-lg font-medium text-gray-900 mb-2">No instruments found</h3>
                                        <p class="text-sm text-gray-500 mb-4">Get started by adding your first laboratory instrument</p>
                                        <button wire:click="openAddModal()"
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div class="hidden sm:block">
                            <p class="text-sm text-gray-700">
                                Showing <span class="font-medium">{{ $instruments->firstItem() ?? 0 }}</span>-<span
                                    class="font-medium">{{ $instruments->lastItem() ?? 0 }}</span> of <span
                                    class="font-medium">{{ $instruments->total() }}</span> instruments
                            </p>
                        </div>
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            <div class="hidden sm:flex items-center space-x-1">
                                @php
                                    $currentPage = $instruments->currentPage();
                                    $lastPage = $instruments->lastPage();
            ...
            <div class="fixed inset-0 bg-gray-900/75 p-4 flex items-center justify-center z-50">
                <div class="relative w-full max-w-md bg-white rounded-2xl shadow-2xl p-8">
                    <div class="flex justify-between items-center pb-4 border-b">
                        <h3 class="text-2xl font-bold">Add New Instrument</h3>
                    ...
                    <div class="mt-6">
                        ...
                        <form wire:submit="store">
                            <div class="mb-5">
                                <label for="add-name" class="block text-sm font-bold mb-2">Instrument Name</label>
                            ...
                            <div class="mb-5">
                                ...
                                <textarea id="add-description" wire:model="description"
                                    ...
                                    placeholder="Enter instrument description..."></textarea>
                                @error('description')
                                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            <div class="flex justify-end pt-5 border-t mt-6">
                                ...
                                <button type="submit" wire:loading.attr="disabled" wire:target="store"
                                    ...
                                    <span wire:loading.remove wire:target="store">Save Instrument</span>
        ...
        @if ($isEditModalOpen)
            <div class="fixed inset-0 bg-gray-900/75 p-4 flex items-center justify-center z-50"
                ...
                <div class="relative w-full max-w-md bg-white rounded-2xl shadow-2xl p-8">
                    <div class="flex justify-between items-center pb-4 border-b">
                        <h3 class="text-2xl font-bold">Edit Instrument</h3>
                    ...
                    <div class="mt-6">
                        ...
                        <form wire:submit="update">
                            <div class="mb-5">
                                <label for="edit-name" class="block text-sm font-bold mb-2">Instrument Name</label>
                            ...
                            <div class="flex justify-end pt-5 border-t mt-6">
                                ...
                                <button type="submit" wire:loading.attr="disabled" wire:target="update"
                                    class="px-6 py-2 rounded-lg text-white bg-blue-500 hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer">
                                    <span wire:loading.remove wire:target="update">Update Instrument</span>
